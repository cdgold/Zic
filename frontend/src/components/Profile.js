...
// MOVE SUBMIT / STOP EDITING TO SUBMIT SONG RATINGS
...
const Profile = ({ otherUser,
  ...
  useEffect(() => {     // fetches album ratings
    ...
    else if(otherUserID === null && user !== null && typeof user !== "undefined" && typeof user.userID !== "undefined"){
  ...
  useEffect(() => {
    if(following === null && user !== null && user.userID !== undefined){
      followerService.getFollowing({ "userID": user.userID })
  ...
  const handleFollow = async () => {
    if (isAuthenticated) {
      ...
      await followerService.follow({ "userID": user.userID, "toFollowID": otherUser["userID"], "token": token })
  ...
  const handleUnfollow = async () => {
    if (isAuthenticated) {
      ...
      await followerService.unfollow({ "userID": user.userID, "toUnfollowID": otherUser["userID"], "token": token })
              ...
              : <>
                {(Array.isArray(following) && typeof (following.find(user => user.userID === otherUserID)) === "undefined") ?
                  <AcceptButton
                    ...
                    text={"Follow"}> </AcceptButton>
                  : <DeclineButton
                    onclick={() => handleUnfollow()}
                    text={"Unfollow"}> </DeclineButton>
                }
              </>
            }
          </div>
